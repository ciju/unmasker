...
(function() {
  var Dragging, GetId, Rects, Selections, initMainMask, log, renderSelectionRects,
    __slice = [].slice,
  ...
  window.RectGroups = (function() {
    ...
    RectGroups.prototype.Delete = function(s) {
      var i, _i, _len, _ref, _results;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        i = _ref[_i];
        if (!s) {
          _results.push(i.Delete());
        } else if (i.id === s.id) {
          _results.push(s.Delete());
        } else {
  ...
  window.SelectionRect = (function() {
    ...
    SelectionRect.prototype.teardownEvents = function() {
      return this.div.off('click dblclick hover');
    };

    SelectionRect.prototype.Remove = function() {
      ...
      _results = [];
      for (i = _i = 0, _len = Selections.length; _i < _len; i = ++_i) {
        s = Selections[i];
        if (!s) {
          continue;
        }
        log('id ', this.id, ' s.id', s.id);
        if (s.id !== this.id) {
          continue;
        }
        log('remove the entry');
        Selections[i] = null;
        _results.push($.publish('selector.removed'));
      }
      return _results;
    };
  ...
  renderSelectionRects = function(sels) {
  ...
  window.doStuff = function() {
    ...
    $.subscribe('selector.removed', function() {
      var i, rects_str;
      ...
      Selections = [];
      initMainMask();
      return renderSelectionRects($.parseJSON('[' + $.localStorage('rects') + ']'));
    });
